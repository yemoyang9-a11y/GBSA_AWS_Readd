 ...
 *  같은 K에서 어느 경로를 타든 투입 레코드가 동일"
 *
 * D13 ②: 모델 선택은 근거 범위에 영향을 주지 않음
 *
 * ⚠️ 2026-08-25 — 난이도(Haiku/Sonnet) 분기를 폐지해 이제 모든 질의가 같은 모델을 쓴다.
 *    그래서 "두 경로의 근거가 같다"는 원래 명제는 더 강한 형태로 성립한다 — 경로가
 *    하나뿐이다. 아래 테스트는 분기 유무와 무관하게 유효한 부분(질의가 달라도 같은 K면
 *    근거가 동일하다)을 그대로 고정한다. 분기를 되살리면 여기에 갈래별 비교를 다시 넣는다.
 */
...
describe('모델 선택 - 근거 집합 불변 (명세 5.5절 #15)', () => {
  ...
  test('명세 5.5절 #15: 질의 성격이 달라도 같은 K의 근거 집합은 동일', async () => {
    // 1. 예전에 Sonnet으로 갈리던 복잡한 질의
    const complexQuery = '왜 정주사와 초봉의 관계가 그렇게 변화했나요? 그리고 앞으로 어떻게 될까요?';
    const complexSelection = selectModel(complexQuery);

    // 2. 예전에 Haiku로 갈리던 단순한 질의
    const simpleQuery = '정주사가 누구인가요?';
    const simpleSelection = selectModel(simpleQuery);

    // 분기 폐지 후: 둘이 같은 모델을 쓴다 (경로가 하나뿐이므로 근거가 갈릴 여지도 없다)
    expect(complexSelection.modelId).toBe(simpleSelection.modelId);

    // 3. 같은 K에서 근거 조립 (질의 텍스트는 assembleContext에 전달 안 됨)
    const contextA = await assembleContext(BOOK_ID, K);
    const contextB = await assembleContext(BOOK_ID, K);

    // 4. 두 호출의 근거가 완전히 동일해야 함
    expect(contextA.chapter_summaries).toEqual(contextB.chapter_summaries);
    expect(contextA.current_chapter_text).toEqual(contextB.current_chapter_text);
    expect(contextA.entities).toEqual(contextB.entities);
    expect(contextA.background).toEqual(contextB.background);
  });

  test('명세 5.5절 #17: 모델 선택이 LLM 호출을 추가하지 않음', () => {
  ...
  test('D13 ②: 모델 선택은 근거 범위(K)를 변경하지 않음', async () => {
    // selectModel은 K를 인자로 받지 않음
    expect(selectModel.length).toBe(1); // query만 인자로 받음

    // 어떤 질의가 들어오든 assembleContext는 같은 K를 사용
    const query1 = '왜 그런가요?';
    const query2 = '누구인가요?';
  ...
  test('근거 조립 → 모델 선택 순서 보장', async () => {
    ...
    // 그 후 selectModel이 query를 받지만 모델만 정한다 (근거엔 손대지 않는다)
    const selection1 = selectModel('왜 정주사와 초봉의 관계가 변화했나요?');
    const selection2 = selectModel('정주사가 누구인가요?');

    expect(selection1.modelId).toBe(selection2.modelId);
