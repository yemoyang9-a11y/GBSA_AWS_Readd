...
- 간결하고 명확하게 답변하세요
- 근거가 명확한 경우에만 답변하세요
- 마크다운 말고 "**"를 진하게 그냥 해줘 마크다운형식을 마크다운 결과로 해줘
- 이모티콘 엄청 많이 쓰고 애교많은 말투로
...
export async function* handleQuery(
  ...
  try {
    ...
    // 놓치지 않는다.
    if (currentPageText) {
      fullPrompt += `\n\n## ⚠️ 지금 보고 있는 페이지 본문 (p.${currentPageText.pageNo}) — 사용자가 실제로 읽고 있는 확정된 텍스트\n\n${currentPageText.content}\n\n(위는 사용자가 지금 화면에 띄워 놓은 페이지 전체입니다. 질문의 답이 이 안에 있으면 "${NO_EVIDENCE_TOKEN}"을 반환하지 말고 반드시 이 내용으로 답하세요 — 이 안의 단어·인명·지명·서술은 전부 이미 열람 가능한 확정된 근거입니다.)`;
    }
    ...
    if (quote) {
      fullPrompt += `\n\n## ⚠️ 사용자가 지금 보고 있는 본문 인용 — 확정된 근거\n\n"${quote}"\n\n(사용자가 읽기 화면에서 직접 선택한 문장입니다. 위 "근거 데이터"·"검색 결과"에 없어도 이 인용문 자체는 "${NO_EVIDENCE_TOKEN}" 반환 없이 답변에 활용하세요.)`;
    }
