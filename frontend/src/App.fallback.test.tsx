...
describe('브리핑 스트리밍 폴백 (mock 기준)', () => {
  ...
  it('저장 리캡이 기준점과 맞으면 그대로 보여주고 폴백을 부르지 않는다', {
    timeout: 20000,
  }, async () => {
    render(<App />);
  ...
  it('진도가 저장분과 어긋나면 폴백 스트림이 화면에 흘러 들어온다', {
    timeout: 30000,
  }, async () => {
    render(<App />);
    ...
    // 저장분(K=20)과 현재 기준점(K=21)이 다르므로 recap: null → 폴백이 흘러 들어온다
    await waitFor(
      () => expect(screen.getByText(/mock 실시간 리캡/)).toBeInTheDocument(),
      { timeout: 10000 }
    );
  });
});
