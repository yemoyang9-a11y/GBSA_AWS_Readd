...
**기준 문서** (`docs` 브랜치) — 설계 시점은 8/18 판(FR/NFR v0.5 · Use Case v0.3 · User Flow v0.3 · MVP v0.5 · B안 확정서 v0.3 · 문제정의서 v0.3 · PRD v0.2 · 변경사항-0818 3차)이며, **8/19에 이 설계의 결과가 상위 문서로 역반영되어 현재 판은 FR/NFR v0.6 · Use Case v0.4 · User Flow v0.4 · MVP v0.6 · B안 확정서 v0.4 · 문제정의서 v0.4 · PRD v0.3이다**(부록 A). 결정 근거는 **8/19 결정 기록(`decisions-0819-final.md`)**
...
파생값 (결정기 전용 함수):
    기준점        cutoff  = 진도 레코드 없음 ? 0 : current_page   (FR-PRG-003, R1)
    진도 %        percent = current_page ÷ total_pages            (MVP 1장)
    완독          is_complete = 진도 레코드 있음 && current_page >= total_pages
    목차상 위치    현재 장 = current_page가 속한 장                (장 경계 테이블)
반환:  스냅샷 { current_page, cutoff, percent, is_complete, chapter }
```

> **[2026-09-01 정정]** 원문은 `cutoff = current_page − 1` 이었다. 구현 과정에서
> **`cutoff = current_page`(진도 레코드가 없을 때만 0)** 로 확정됐다(R1, FR-PRG-003).
> `−1` 은 "열어본 페이지는 아직 안 읽었다"는 전제였는데, 실제로는 열린 페이지까지를
> 읽은 것으로 보는 편이 리캡·챗봇의 체감과 맞았다. 첫 진입(K=0)만 예외로 남는다.
>
> 완독 파생값 `is_complete` 는 **D14(2026-08-27 팀 결정)** 로 추가됐다. 챗봇이 근거에
> 없는 대상을 "이 책에 나오지 않아요"로 단정할 수 있는 유일한 예외의 근거이며(R10),
> 다른 어떤 코드도 `page >= total_pages` 를 재계산하지 않는다.
...
단일 반응형 웹앱, **태블릿 PC 우선**(8/18 결정 — NFR-PLT-001·002 v0.6에서 반영 완료). 태블릿 가로 폭에서 읽기 화면과 싸비 사이드창의 동시 표시를 기본 레이아웃으로 설계하고, 모바일은 축소 대응으로 내린다. 화면 4종 + 오버레이 2종.
...
> **⚠️ [2026-09-01 정정] 이 절은 설계된 동작이며, 스위퍼는 현재 가동되지 않는다.**
>
> 로직 자체는 구현돼 있으나 **배선이 없다.** `runSweep` 을 호출하는 것은 수동 실행용
> `run-sweep.ts` 하나뿐이고, `index.ts` 에 스케줄러가 없으며 프로덕션에도 crontab·
> cron.d·systemd timer 가 전부 비어 있었다(2026-08-29 실조회 확인).
>
> 방증 — 모든 `reading_session` 행의 `recap_state` 가 `none` 이다. 스위퍼가 한 번도
> 돌지 않았다는 뜻이고, 따라서 **저장 리캡이 생성된 적이 없다.** `last_activity_at` 은
> `touchActivity` 가 5개 경로에서 정상 갱신하지만 그 값을 읽는 쪽이 없다.
>
> 결과적으로 **R7(세션 종료 시 저장 리캡 재종합)은 돌지 않으며, 흐름 B의 저장 리캡
> 재사용 경로도 사실상 죽어 있다**(5.2절 정정 참조). 진입 판정은 `SESSION_TIMEOUT_MS`
> 를 참조하지 않고 항상 새 세션으로 처리한다 — 이는 버그가 아니라 **D-3(migration.md)
> 으로 정식 결정된 동작**이다. 포트폴리오에서는 방문자가 반드시 브리핑을 거쳐야 제품이
> 설명되기 때문이다. 30분 규칙만 되살리면 스위퍼가 여전히 미동작이라 재방문자가
> 브리핑도 저장 리캡도 못 보는 상태가 되어 명세 정합성이 오히려 나빠진다.
...
| 임베딩 | **모델 확정으로 해소 [8/19 — A8-1·D12]** — Cohere embed-v4 (1024차원), 표본 비교 생략 (5.3절). ~~Amazon Titan Text Embeddings V2~~ 에서 **2026-08-30 이관** |
...
> **⚠️ [2026-09-01 정정] "저장 리캡 재사용"이 주 경로라는 전제는 현재 성립하지 않는다.**
>
> 위 조립 규칙은 그대로 유효하지만, **재사용 판정 두 줄 중 첫 줄(저장 리캡)은 실질적으로
> 도달하지 않는다.** 저장 리캡을 만드는 주체가 세션 종료 스위퍼인데 그 스위퍼가 배선되지
> 않아 한 번도 돈 적이 없기 때문이다(4.4.1절 정정). 즉 **모든 리캡 요청이 실시간 생성
> 경로를 탄다.**
>
> 남는 재사용은 **세션 캐시(K)** 하나다(UC-09 A7). 이건 정상 동작한다 — 같은 세션에서
> 같은 K로 다시 요청하면 완성 텍스트를 단일 delta로 흘린다. 2026-08-30 검증에서 리캡
> delta가 1개로만 와서 스트리밍이 깨진 줄 알았던 적이 있는데, K=40에 세션 캐시가 있어
> 이 경로를 탄 것이었다 — **깨진 게 아니라 R8/UC-09 A7이 정상 동작한다는 증거였다.**
>
> 여기에 D-3(항상 새 세션 → 항상 브리핑)이 겹쳐, 실제 흐름은
> **매번 새 세션 → 매번 브리핑 → 항상 실시간 스트리밍** 이다. 저장 리캡을 되살리려면
> 스위퍼 배선이 선행 조건이다.
...
| 중첩 | 없음 (페이지 1:1 원칙) |
| 임베딩 모델 | **Cohere embed-v4 (1024차원) [2026-08-30 이관 — migration.md D-1]** — 다국어 지원. 원래는 Amazon Titan Text Embeddings V2 (1024차원)였고(8/19 확정 — A8-1·D12), AWS 이탈로 교체했다. **출력 차원을 1024로 맞춰 재임베딩만으로 끝냈다** — 차원이 달랐다면 HNSW 인덱스와 스키마까지 손봐야 했다 |
...
| ~~A8-1~~ | **해소 (8/19 — D12)** — 표본 비교 생략하고 표준안 확정. 모델은 **Cohere embed-v4 (1024차원)** 로 2026-08-30 이관(원래 Amazon Titan Text Embeddings V2) | 5.3 | 확정 |
...
# 부록 A. 상위 문서 개정 지점 (8/19 확정 목록) — **반영 완료**

v0.2 부록의 정합성 메모(5건)를 8/19 결정 기록(`decisions-0819-final.md` 4장)의 확정 개정 목록으로 교체한다.

> **아래 개정은 8/19에 전부 반영됐다.** 반영 후 판 — FR/NFR **v0.6** · Use Case **v0.4** · MVP **v0.6** · PRD **v0.3** · B안 확정서 **v0.4** · 문제정의서 **v0.4** · User Flow **v0.4**. 이 표는 무엇을 왜 고쳤는지의 기록으로 유지한다.
>
> **반영하지 못한 것 1건** — **프로젝트 지침 6·15장**(진도 기준점이 최대 도달 위치(watermark)로 서술되어 FR-PRG-003과 정반대). 이 저장소 밖 문서라 손대지 못했다. 개발자가 먼저 읽는 문서이므로 방치하면 FR-PRG-003 게이트의 4개 케이스 중 3개가 실패한다 — MVP 7장 선결 1번으로 여전히 최우선이다.
...
| NFR-PLT-001·002 | "모바일 우선" → "태블릿 PC 우선(모바일 축소 대응)" — **반영 완료** |
