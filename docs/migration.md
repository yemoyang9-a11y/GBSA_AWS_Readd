...
## Phase 3 — 데이터 이전 (2026-08-30, **미완료 — 접속 정보 대기**)

### 상태

**실행하지 못했다.** Supabase 접속 정보와 Cohere 키가 아직 로컬에 없다
(`DATABASE_URL`은 여전히 localhost, `COHERE_API_KEY` 미설정). 자격증명이 도착하면
바로 돌릴 수 있도록 준비만 해 뒀다.

| 단계 | 상태 |
| --- | --- |
| 1. `CREATE EXTENSION vector` | 대기 — 스크립트에 포함 |
| 2. 덤프 복원 + 20개 테이블·행 수 대조 | 대기 — 스크립트에 포함 |
| 3. HNSW 인덱스 확인 | 대기 — 없으면 생성하도록 스크립트에 포함 |
| 4. Titan 임베딩 무효화 | 대기 — 스크립트에 포함 |
| 5. Cohere 재임베딩 | 대기 — **스크립트를 배치로 고쳤다(아래)** |
| 6. 차원 확인 (1024) | 대기 |
| 7. pooler 커넥션 검증 | 대기 |

`backend/scripts/migrate-to-supabase.sh` — 1~4단계를 한 번에 돌린다. 접속 정보는
`SUPABASE_DB_URL` 환경변수로만 받고 파일에 남기지 않는다. 4단계 무효화 전에 5초
대기와 경고를 넣었다(되돌릴 수 없는 단계다).
...
⚠️ **이 수치는 리전 검증이 아니다.** 로컬 Docker Postgres 기준이라 네트워크 왕복이
사실상 0이다. 지시서 7번("리전이 어긋났다면 여기서 드러난다")의 목적은 Supabase에
붙어야 달성된다 — Phase 3 이후 다시 측정해야 한다.

### 9. 벡터 검색 — **미수행** (Cohere 키 필요)

질의 임베딩을 만들 수 없어 검색 자체를 돌리지 못했다.
...
- 리캡 생성 (SSE 첫 토큰 도착, 하드 상한 도달 시 `gen.return()` 조기 취소 안전성)
- 챗봇 응답 (근거 조립, R10 고정 문구 수렴)
- NFR-SEC-006 인젝션 10건
- 데모 시나리오 (p.40 거절 → p.80 응답)
