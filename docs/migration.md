...
위 세 항목은 대부분 해소됐다. 이 절은 Phase 1 시점의 기록이므로 고치지 않고 결과만 잇는다.

| 항목 | 결과 |
| --- | --- |
| **A8-1 한국어 표본 검증** | **해소.** Phase 4 벡터 검색에서 실증됐다 — 질의 3종 모두 1위가 기대 장 구간 안이고 1위 페이지 원문이 실제 그 장면이었다(형보 질의 1위 p397). 별도의 20~30문장 유사도 비교를 따로 돌릴 필요가 없어졌다 |
| **실제 호출 검증** | **대부분 해소.** Phase 4에서 리캡 스트리밍(delta 15개 점진 도착)·챗봇 상한(K=40/80 차이)·R10 수렴·로그 적재를 실호출로 확인했다. **다만 `gen.return()` 조기 취소는 여전히 확인하지 않았다** — 아래 참조 |
| **AWS 배포 산출물** | `deploy-backend.yml` 은 Phase 2 에서 Fly 배포로 교체됐다. `infra/terraform/` 은 **지우지 않고 종료 표시로 마무리**했다 — 아래 참조 |

**⚠️ `gen.return()` 조기 취소는 미검증으로 남는다.** 하드 상한 도달 시 제너레이터를 조기
종료하는 경로가 Anthropic SDK 에서 안전한지 명시적으로 확인한 적이 없다. Bedrock 에서는
이 취소가 내부 예외로 번져 응답 전체를 500 으로 만든 전례가 있어 호출부가 try/catch 로
삼키고 있다. 실사용에서 문제가 드러나지 않았을 뿐 **검증된 것은 아니다.**

**`infra/terraform/` 은 남긴다.** 이전 전 인프라를 **코드로 기술한 유일한 산출물**이라
지우지 않고, [`infra/terraform/README.md`](../infra/terraform/README.md) 에 종료 사실과
`terraform apply` 금지를 명시했다. `architecture-aws.md`(실조회 기록)·`architecture-r2.md`
(설계 의도)와 성격이 달라 셋이 서로를 대체하지 않는다.

그 README 를 쓰면서 **문서 간 불일치를 하나 발견했다** — `architecture-aws.md` 는 실제
인프라에 Elastic IP 2개가 있었다고 기록하는데 Terraform 에는 `aws_eip` 리소스가 없다.
Terraform 밖에서 따로 만들어 붙였거나 구성이 도중에 어긋난 것이다. **계정이 이미 닫혀
확인할 수 없으므로 어느 쪽인지 단정하지 않고 사실만 남긴다.**
...
## Phase 5 — AWS 정리 (2026-08-31, **우리 손을 떠남**)

**이 단계는 수행하지 않았다.** 교육 과정 관리자가 리소스를 먼저 중단시켰고 계정
자격증명도 무효화됐다 — `aws sts get-caller-identity` 부터 접근이 끊겼다(실행 확인).
삭제 목록을 만들어 승인받고 지우는 절차 전체가 빠졌고, 비가역 지점이 하나 사라졌다.

**잃은 것은 없다.** Phase 0 에서 미리 떠 둔 백업이 로컬에 온전하다 — `backup/ssabi.dump`
2.8MB(`PGDMP` 헤더 정상), S3 객체 14개, 인프라 구성 JSON 21개. 더 중요한 건 **Supabase 가
이미 정본**이라는 것이다. 20개 테이블에 임베딩 411/411 이 1024차원으로 들어가 있어 덤프
없이도 서비스가 돈다. 백업은 이제 의존 대상이 아니라 보험이다.

**다만 되돌아갈 길도 같이 사라졌다.** 이전에 문제가 생겨도 AWS 로 롤백할 수 없다.
실무적으로는 큰 차이가 없다 — 로컬 덤프를 다른 Postgres 에 복원하면 되고,
[`architecture-aws.md`](architecture/architecture-aws.md)와 구성 JSON 에 옛 인프라가
기록돼 있어 포트폴리오 서술도 가능하다. 애초에 그 목적으로 떠 둔 것이다.

교훈 — **되돌릴 수 없는 단계의 백업을 "나중에"로 미루지 않은 것이 결정적이었다.**
백업을 뜬 8/29 시점에는 계정이 언제 닫힐지 몰랐고, 실제로 이틀 뒤 예고 없이 닫혔다.

---

## Phase 6 — 문서 동일화 (2026-09-01)

문서와 실제를 맞추는 단계다. 코드 변경은 없다.

### 무엇을 했나

**[`architecture-current.md`](architecture/architecture-current.md) 신규 작성.**
이전 후 인프라를 실조회·실측값으로 기록했다 — Pages 빌드 설정과 환경변수, Fly 머신·
스케일-투-제로·시크릿·CORS, Supabase Session pooler 와 자체 루트 CA, Anthropic·Cohere,
배포 파이프라인, 비용, 실측 성능표. 9장에 AWS 구성과의 차이를, 10장에 **확인하지 못한
것**을 따로 뒀다(Supabase 무료 등급 한도, 재배포 시 머신 개수).

**[`architecture-r1.md`](architecture/architecture-r1.md) 정정 7건.** 이 문서는 로그가
아니라 **참조 문서**라 사실을 본문에서 고치되, 설계 이력이 지워지지 않게 `[2026-09-01
정정]` 표시를 남겼다.

| 대상 | 내용 |
| --- | --- |
| 3.3절 기준점 정의 | `cutoff = current_page − 1` → **`진도 레코드 없음 ? 0 : current_page`** (R1) |
| 3.3절 스냅샷 | 완독 파생값 `is_complete` 추가 (D14) |
| 4.4.1 스위퍼 | **미가동** 명시 — 배선 없음, `recap_state` 전 행 `none`, R7 미동작 |
| 5.2 흐름 B | "저장 리캡 재사용이 주 경로" 전제 정정 — 매번 새 세션 → 매번 브리핑 → 항상 실시간 스트리밍 |
| 406·605·760행 | Amazon Titan → **Cohere embed-v4 (1024차원)** |

### 왜 그렇게 했나

**r1 은 참조 문서라 append 방식이 맞지 않는다.** `migration.md` 는 시간순 기록이라
정정을 뒤에 쌓지만, r1 은 사람이 "지금 시스템이 어떻게 도는가"를 찾아 읽는 문서다.
기준점 정의가 틀린 채로 남으면 누군가 그대로 구현한다. 그래서 본문을 고치고 인용 블록에
"원문은 무엇이었고 왜 바뀌었나"를 붙였다.

**`cutoff = current_page − 1` 이 바뀐 이유**도 함께 적었다. `−1` 은 "열어본 페이지는
아직 안 읽었다"는 전제였는데, 실제로는 열린 페이지까지를 읽은 것으로 보는 편이 리캡·
챗봇의 체감과 맞았다. 첫 진입(K=0)만 예외로 남는다.

### 검증

문서 간 상대 링크를 파일 존재로 확인했다 — **12건 전부 해석, 미해석 0건.** 통합 전에
깨져 있던 `architecture-aws.md → architecture-r1.md` 와 `→ architecture-current.md` 가
둘 다 살아났다.

### 아직 못 한 것

- **README 신규 작성** — 저장소 첫 화면이 아직 이전 전 서술이다
- `frontend/.nvmrc`, `FLY_API_TOKEN`, `main` 병합 후 Pages 브랜치 변경 (위 「남은 것」)
